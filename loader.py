...
class MoleculeDataset_scf(InMemoryDataset):
    def __init__(self,
                 root,
                 #data = None,
                 #slices = None,
        ...
        super(MoleculeDataset_scf, self).__init__(root, transform, pre_transform,
                                                 pre_filter)
        self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            all_scaffolds = dict(sorted(all_scaffolds.items(), key=lambda x: (len(x[1]), x[1][0]), reverse=True))

            molId_scfId = np.empty(len(smiles_list))
            scf_scfId = dict()
            for i, (scf, ids) in enumerate(all_scaffolds.items()):
                if scf == '':
                    scf = 'blank'
                molId_scfId[ids] = i
                scf_scfId[scf] = [i, len(ids)]
            ...
            for i, smile in enumerate(smiles_list):
                ...
                #Chem.SanitizeMol(rdkit_mol,
                                 #sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
                data = mol_to_graph_data_obj_scf(rdkit_mol, smile)
                # manually add mol id
                data.id = torch.tensor(
                    [i])  # id here is the index of the mol in
                # the dataset
                data.y = torch.tensor(labels[i, :])
                data.scf = molId_scfId[i]
                # data.smile = smiles_list[i]
                data_list.append(data)
                data_smiles_list.append(smile)

            with open(os.path.join(self.processed_dir,'scaffold_index.csv'), 'w') as f:
                w = csv.writer(f)
                for k, v in scf_scfId.items():
