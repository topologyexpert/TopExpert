...
class GNN_topexpert(torch.nn.Module): # 
    ...
    def __init__(self, args, criterion):
        ...
        self.experts = nn.ModuleList([expert(args.emb_dim, self.num_tasks) for i in range(self.num_experts)])
    ...
    def forward(self, data):
        ...
        ## multi-head mlps
        clf_logit = [self.experts[i](gnn_out) for i in range(self.num_experts)]
        clf_logit = torch.stack(clf_logit, dim=-1) # N  x emb_dims x num_experts
